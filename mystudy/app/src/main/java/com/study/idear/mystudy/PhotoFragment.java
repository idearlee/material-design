...
public class PhotoFragment extends Fragment {
    ...
    private static PhotoFragment mInstance;

    public static PhotoFragment getmInstance() {
        if (mInstance == null) {
            mInstance = new PhotoFragment();
        }
        return mInstance;
    }
    ...
    private void initRecycle() {
        ...
        mSwip.setColorSchemeColors(Color.CYAN);
    }

    private void initRecycleData() {
        mRecycleData = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            mRecycleData.add("波多野结衣" + i);
        }
    }

    public void addData() {
        if (mRecycleData != null) {
            mRecycleData.add("吉泽明步");
        }
